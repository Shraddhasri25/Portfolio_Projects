...
def setTime():
    ...
    greenTime = 22  # Set a fixed green time of 60 seconds
...
def printLaneCounts():
    lane_counts = [0, 0, 0, 0]  # Initialize counts for each lane
    for i in range(noOfSignals):
        for lane in range(3):
            lane_counts[lane] += vehicles[directionNumbers[i]][lane]['crossed']
    total_count = sum(lane_counts)
    lane_counts.append(total_count)
    print(f'{lane_counts[0]:>5} {lane_counts[1]:>5} {lane_counts[2]:>5} {lane_counts[3]:>5} {total_count:>5}')

# Modify the repeat function to include printing counts
def repeat():
    for _ in range(12):  # Repeat 12 times (1 hour)
        ...
        printLaneCounts()  # Print lane counts after each cycle
